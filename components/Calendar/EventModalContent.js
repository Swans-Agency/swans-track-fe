  ...
  return (
    <div className="flex flex-col">
      ...
      <p className="text-md font-light">
        {moment(selectedItem?.start?.dateTime).format("dddd")}{" "}
        {moment(selectedItem?.start?.dateTime).format("MMMM Do, YYYY")}
      </p>
      <p className="text-md font-light">
        {moment(selectedItem?.start?.dateTime).format("HH:mm")} -{" "}
        {moment(selectedItem?.end?.dateTime).format("HH:mm")}
      </p>
      {selectedItem?.hangoutLink && (
        <div className="text-md font-light flex items-center gap-x-1 py-2">
          <img
            src={selectedItem?.conferenceData?.conferenceSolution?.iconUri}
