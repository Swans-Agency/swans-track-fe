...
export default function DayHeader({ date, isConnected }) {
    ...
    const onFinish = async (values) => {
        console.log(values);
        values.startTime = moment(date).format("YYYY-MM-DD") + moment(values.startTime).format("[T]HH:mm:ss")
        values.endTime = moment(date).format("YYYY-MM-DD") + moment(values.endTime).format("[T]HH:mm:ss")
        values.recurrenceTo = moment(values.recurrenceTo).format("YYYY-MM-DD[T]HH:mm:ss")
        console.log(values);
    ...
    return (
        <>
            ...
            <ModalANTD
                title={isConnected?.details ? `Create event on ${moment(date).format('ddd')} ${format(date, 'MMM dd, yyyy ')}` : ""}
                footer={null}
                isModalOpen={isModalOpen}
                handleOk={handleOk}
                handleCancel={handleCancel}
                renderComponent={<CreateJoinEvent isConnected={isConnected} form={form} onFinish={onFinish} />}
            />
