...
export default function IncomeForm({ setReloadData }) {
    ...
    const searchProposal = async (value) => {
        ...
        const arrData = proposalSearchData?.map((item) => ({
            value: item.id,
            label: `${item?.invoiceNo} | ${item?.toCompanyName} | ${item?.invoiceDate} | ${Number(item?.invoiceTotal).toFixed(2)} JD`,
        }));
    ...
    return (
        ...
        >
            <div className='flex gap-x-5 w-full mt-0'>
                <Form.Item label="Income Description" name="description" className='w-full' required>
                    <Input className='rounded' />
                </Form.Item>
                <Form.Item label="Payment Amount" name="amount" className='w-full' required>
                    <InputNumber min={0} className='rounded w-full' addonAfter={<MoneyCollectOutlined className='pb-1' />} />
                </Form.Item>
            </div>
            <div className='flex gap-x-5 w-full mt-0'>
                <Form.Item label="Payment Method" name="paymentMethod" className='w-full' required>
                    <Select
                        showSearch
                        defaultValue=""
                        style={{
