...
export default function TaskForm({
  ...
  const getAllEmployees = async () => {
    ...
    let pathname = router.pathname.startsWith("/invited-project")
      ? true
      : false;
  ...
  const handleInitialValues = () => {
    if (selectedItem) {
      ...
      form.setFieldValue(
        "taskDescription",
        selectedItem?.taskDescription !== "undefined"
      ...
      selectedItem?.priority !== "undefined" &&
        form.setFieldValue("priority", selectedItem?.priority);
      selectedItem?.dueDate &&
        form.setFieldValue(
          "dueDate",
          dayjs(new Date(selectedItem?.dueDate ? selectedItem?.dueDate : null))
        );
      selectedItem?.startDate &&
        form.setFieldValue(
          "startDate",
          dayjs(
            new Date(selectedItem?.startDate ? selectedItem?.startDate : null)
          )
        );
      {
            ...
            label: (
              <>
                <div className="flex items-center gap-x-2">
                  <Image
                    src={
                      selectedItem?.assignee?.pfp?.split("?")[0] ||
                      "/whiteLogo.svg"
                    }
                    width={20}
                    height={20}
    ...
    if (selectedItem) {
      if (data?.assignee == undefined) {
        ...
        if (Number.isNaN(Number(assigneeValue))) {
          formData.append(
            "assignee",
            assigneeValue ? selectedItem?.assignee?.id : null
          );
        } else {
  ...
  const handleGetStatus = async () => {
    ...
    let data = await getAxios(url, false, false, (data)=>{
      let statusesArr = data?.map((item) => {return ({label: item?.columnName, value: item?.columnName})})
      setTaskStatuses(statusesArr)
    }, false)
    ...
    <div>
      ...
      >
        <div
          className={`${
            selectedItem
              ? "grid laptop:grid-cols-2 phone:grid-cols-1 gap-x-12 justify-between w-[100%]"
              : ""
          }`}
        >
          <div>
                ...
                {showInput && (
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <div className="flex gap-1">
                      <div
                        className="bg-mainBackground dark:bg-[#141414] rounded-lg text-white hover:shadow-lg hover:cursor-pointer px-3 py-2"
                        onClick={() => handleCreateChecklist()}
                      >
                        Save
                      </div>
                      <div
                        className="bg-gray-400 dark:bg-[#282828] rounded-lg text-white hover:shadow-lg hover:cursor-pointer px-3 py-2"
                        onClick={() => handlehideInput(false)}
                      >
                        Cancel
                      </div>
                    </div>
            ...
            <div className="flex gap-x-5 w-full mt-0">
              <Form.Item label="Start Date" name="startDate" className="w-full">
                <DatePicker size="large" className="w-full" placeholder="" />
              </Form.Item>
              <Form.Item label="Due Date" name="dueDate" className="w-full">
                <DatePicker
                  size="large"
                  className="w-full"
                  placeholder=""
                  disabledDate={(current) => {
                    ...
                    return (
                      current &&
                      (current < moment().startOf("day") || current < startDate)
                    );
                  }}
                />
              </Form.Item>
            </div>
            <Form.Item label="Assignee" name="assignee" className="w-full">
              <Select
                size="large"
                showSearch
                defaultValue=""
          ...
          <div>
            {selectedItem && (
              <div>
                ...
                Object.keys(selectedItem?.history || {}).length ? (
                  <div className="dark:bg-[#141414] px-4 py-[0.60rem] rounded-lg dark:border dark:border-[#424242]  hover:border-blue-400">
                    {showHistory ? (
                      <div className="max-h-[250px] overflow-scroll">
                        {selectedItem &&
                          selectedItem?.history &&
                          Object?.entries(selectedItem?.history)?.map(
                            (item) => {
                              return (
                ...
                {selectedItem && showHistory && (
                  <div
                    ...
                    className="flex justify-center mt-2 w-full bg-gray-400 dark:bg-[#282828] text-center py-2 rounded-lg hover:shadow-lg hover:cursor-pointer"
                  >
                    Close
                  </div>
                )}
            ...
            {selectedItem && (
              <div className="mt-4">
                ...
                <>
                  {Object.keys(selectedItem?.comments || {}).length ? (
                    Object?.entries(selectedItem?.comments)?.map((item) => {
                      return (
                        <div className="dark:text-[#b3b3b3] mb-2 flex items-center gap-3">
                          <div className="bg-[#282828] text-center flex items-center justify-center rounded px-2 py-1">
                            <p title={item[1]?.user} className="font-bold">
                              {item[1]?.user?.substring(0, 2)}
                            </p>
                          </div>
                          <div>
                            <p className="font-bold">{item[1]?.comment}</p>
                            <p className="text-xs text-gray-500">
                              {dayjs(item[0]).format("DD-MM-YYYY HH:mm")}
                            </p>
                          </div>
                        </div>
                      );
                  ...
                  <div className="flex gap-x-2">
                    <div className="bg-[#282828] text-center flex items-center h-fit justify-center rounded px-2 py-1">
                      {cookie
                        .load("username", { path: "/" })
                        ?.substring(0, 2)
                        ?.toUpperCase()}
                    ...
                    <div className="w-full">
                      ...
                      <button
                        ...
                        className={`flex justify-center mt-2 w-fit bg-gray-400 dark:bg-[#282828] text-center py-2 px-4 rounded-lg  ${
                          !commentText?.length
                            ? "cursor-not-allowed"
        ...
        <div className="flex gap-x-5 w-full justify-between">
          ...
          <div className={`${!selectedItem ? "w-full flex justify-end" : ""}`}>
            <Form.Item>
              <FormButtons
                content={selectedItem ? "Save" : "Create"}
                isLoading={isLoading}
              />
            </Form.Item>
          </div>
