...
export default function index({ accessToken, userPermission, userId }) {
  ...
  const handleDrop = async (event, destinationTable) => {
    ...
    console.log({
      handleDrop: { item: item, destinationTable: destinationTable },
    });
    ...
    if (
      ...
      destinationTable !== item?.taskStatus
    ) {
      ...
      const data = {
        taskStatus:
          destinationTable == "toDo"
            ? "To Do"
            : destinationTable == "idle"
            ? "Idle"
            : destinationTable == "completed"
            ? "Completed"
            : destinationTable == "inProgress"
            ? "In Progress"
            : item?.taskStatus,
      };
      ...
      handleNotifyTeam(
        item.assignee,
        `The Task ${item?.taskName} was moved to On Going`
      );
    }
  };
  ...
  return (
    <>
      {itemsToDelete.length > 0 && (
        <div className="absolute  top-0 text-center left-0 w-[100%] h-14 flex  items-center justify-around bg-gray-500 !z-[10000000000000]">
          ...
          <div
            ...
            className="flex gap-x-2 justify-center items-center text-white hover:cursor-pointer"
          >
            <svg
      ...
      <div>
        ...
        {showDelete && (
          <div
            ...
            onDrop={(event) => handleDrop(event, "delete")}
            onDragOver={(event) => handleDragOver(event)}
            className="animate-jump fixed bottom-2 left-[58%] -translate-x-1/2 w-[50%] p-4 border rounded-xl border-dashed border-red-600 bg-red-50 opacity-75"
          >
